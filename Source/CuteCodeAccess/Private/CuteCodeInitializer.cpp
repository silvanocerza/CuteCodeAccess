...
void FCuteCodeInitializer::CreateOthersPriFile() const
{
    TArray<FString> OthersPriLines{{
        "######################################################################",
        "########## This file has been generated by CuteCodeAccess ############",
        "########## DO NOT EDIT IF YOU DON'T KNOW WHAT YOU'RE DOING ###########",
        "######################################################################",
    ...
    AppendFormattedStrings(OthersPriLines, "\"{0}\" \\", ProjectVisitor.GetOtherFiles());

    FString OthersPriFilePath = FPaths::Combine(
        SolutionPath,
        INTERMEDIATE_PROJECTFILES,
        FString{"others.pri"}
    );

    FFileHelper::SaveStringArrayToFile(OthersPriLines, *OthersPriFilePath);
}
